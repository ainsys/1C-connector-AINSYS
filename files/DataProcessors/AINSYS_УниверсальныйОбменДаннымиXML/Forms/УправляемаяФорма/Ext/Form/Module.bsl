...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		...
		ИмяСохраняемогоФайла = ?(Объект.АрхивироватьФайл, НСтр("ru = 'AINSYS описание данных.zip'"),НСтр("ru = 'AINSYS описание данных.xml'"));
...
#Область lyayДоработкиДляAINSYS
 
&НаСервере
Функция ПравилаНастроены()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ Первые 1
	|	AINSYS_ОписаниеДанных.Ссылка
	|ИЗ
...
&НаСервере
Функция lyayПравилаКонвертацииВоВременноеХранилище()
	Объ = РеквизитФормыВЗначение("Объект");
	Макет = Объ.ПолучитьМакет("AINSYS_ПравилаВыгрузкиНастроек");
	АдресФайлаПравилВХранилище = ПоместитьВоВременноеХранилище(Макет, ЭтаФорма.УникальныйИдентификатор);
	
	Возврат АдресФайлаПравилВХранилище;
	//ВыполнитьЗагрузкуПравилОбмена(Адрес, ВыбранноеИмяФайла);
КонецФункции

&НаКлиенте
Процедура lyayПриОткрытии(Отказ)
	ИзменитьРежимОбработки(Истина);
	Если ПравилаНастроены() Тогда
		Элементы.ГлавнаяПанельФормы.ТекущаяСтраница = Элементы.ГлавнаяПанельФормы.ПодчиненныеЭлементы.Выгрузка;
	Иначе 
		Элементы.ГлавнаяПанельФормы.ТекущаяСтраница = Элементы.ГлавнаяПанельФормы.ПодчиненныеЭлементы.Загрузка;
	КонецЕсли;
	
	АдресФайлаПравилВХранилище = lyayПравилаКонвертацииВоВременноеХранилище();
	ПрочитатьПравилаОбменаЗавершение(Истина, АдресФайлаПравилВХранилище, ".xml", Неопределено);
	// ВыполнитьЗагрузкуПравилОбмена(АдресФайлаПравилВХранилище, ".xml");
	//lyayЗагрузитьПравилаКонвертации();
КонецПроцедуры
