...
//  ОбъектВладелец - ОбработкаОбъект.AINSYS_УниверсальныйОбменДаннымиXML - объект, который будет являться владельцем 
...
// Возвращает проинициализированную обработку AINSYS_УниверсальныйОбменДаннымиXML базы-приемника,
...
// 
//  Возвращаемое значение:
//    ОбработкаОбъект.AINSYS_УниверсальныйОбменДаннымиXML - обработка базы-приемника для загрузки данных в базу-приемник.
//
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Попытка
		РезультатПодключения = ОбъектПодключения.Обработки.AINSYS_УниверсальныйОбменДаннымиXML.Создать();
	Исключение
...
Функция ЗначениеСсылочногоТипа(Значение)
	...
	Тип = ТипЗнч(Значение);
	// [lyay тут поправил
	// Справочники.ТипВсеСсылки().СодержитТип(Тип) возвращает Ложь для справочников из расширения
	// переделал
	ТекСтруктураСвойств = Менеджеры[Тип];
	Если ТекСтруктураСвойств = Неопределено ИЛИ Тип = Тип("Неопределено") Тогда
		Возврат Ложь;
	Иначе
		ЭтоСсылка = Метаданные.Справочники.Содержит(ТекСтруктураСвойств.ОбъектМД) ИЛИ
		Метаданные.Документы.Содержит(ТекСтруктураСвойств.ОбъектМД) ИЛИ
		Метаданные.Перечисления.Содержит(ТекСтруктураСвойств.ОбъектМД) ИЛИ
		Метаданные.ПланыВидовХарактеристик.Содержит(ТекСтруктураСвойств.ОбъектМД) ИЛИ
		Метаданные.ПланыСчетов.Содержит(ТекСтруктураСвойств.ОбъектМД) ИЛИ
		Метаданные.ПланыВидовРасчета.Содержит(ТекСтруктураСвойств.ОбъектМД) ИЛИ
		Метаданные.БизнесПроцессы.Содержит(ТекСтруктураСвойств.ОбъектМД) ИЛИ
		Метаданные.Задачи.Содержит(ТекСтруктураСвойств.ОбъектМД) ИЛИ
		Метаданные.ПланыОбмена.Содержит(ТекСтруктураСвойств.ОбъектМД);
		Возврат ЭтоСсылка;
	КонецЕсли; 
	//]
