...
Функция ДеревоВМассив(Дерево, Колонки, стркВышестоящиеЗначения = Неопределено, Уровень = 0) Экспорт
	...
	Для Каждого строкаДерева из Дерево.Строки Цикл
		...
		//БылиПовторяющиесяЗначения = Ложь;
		Для Каждого Колонка из Колонки Цикл
			Если НЕ ЗначениеЗаполнено(строкаДерева[Колонка.Имя]) Тогда
				Продолжить;
			КонецЕсли;
			//Если НЕ БылиПовторяющиесяЗначения Тогда
				//времСвойство = Неопределено;
				//стркВышестоящиеЗначения.Свойство(Колонка.Имя, времСвойство);
				//Если НЕ времСвойство = строкаДерева[Колонка.Имя] Тогда
					//стркВышестоящиеЗначения.Вставить(Колонка.Имя, строкаДерева[Колонка.Имя]);
					стрк.Вставить(Колонка.Имя, строкаДерева[Колонка.Имя]);
				//Иначе
				//	БылиПовторяющиесяЗначения = Истина;
				//КонецЕсли;
			//Иначе
			//	стрк.Вставить(Колонка.Имя, строкаДерева[Колонка.Имя]);
			//КонецЕсли;
...
Функция ОписаниеДоступаКМетодам1С(БазовыйУрл) Экспорт
	Описание = "Доступ к сущностям по запросу " + БазовыйУрл + "/data/ИДЕНТИФИКАТОР/ФОРМАТ/КОЛИЧЕСТВО/СМЕЩЕНИЕ?ПараметрыЗапроса, где
	...
	|
	|	ПараметрыЗапроса:
	|	k - ключ плана синхронизации. Обязательный. Он задётся в справочнике AINSYS_ПланыСинхронизации.
...
// Тип - наименование вида справочника, документа или плана обмена. Благо их наименования не пересекаются.
Функция ПолучитьСсылкуПоИД(ИД, Тип = Неопределено) Экспорт
	...
	Если Тип <> Неопределено Тогда
		...
		Если спр <> Неопределено Тогда
			...
			Если Не Ссылка.Пустая() и НЕ ПустаяСтрока(Ссылка.ВерсияДанных) Тогда // Ссылка.ПолучитьОбъект() <> Неопределено
				Возврат Ссылка
			КонецЕсли;
			Возврат Неопределено
		КонецЕсли;
	...
	Для Каждого спр из Метаданные.Справочники Цикл
		...
		Если НЕ ПустаяСтрока(Ссылка.ВерсияДанных) Тогда // Ссылка.ПолучитьОбъект() <> Неопределено Тогда
			Возврат Ссылка
		КонецЕсли;
	КонецЦикла;
