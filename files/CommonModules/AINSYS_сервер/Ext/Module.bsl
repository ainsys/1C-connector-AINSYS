...
Функция ПодготовитьЗначениеДляJSON(Свойство, Значение, ДополнительныеПараметры, Отказ) Экспорт
	ТипПолученногоЗначения = ТипЗнч(Значение);
	
	// ВНИМАНИЕ! ТипВсеСсылки() не возвращает типы данных расширений
	// Поэтому справочники из расширений будут передаваться в виде представлений
	// Это грустно и печально. Может быть 1С решит проблемку.
	Если Значение = Null Тогда
		рез = "";
	ИначеЕсли ТипПолученногоЗначения = Тип("Строка")
				ИЛИ ТипПолученногоЗначения = Тип("Число")
				ИЛИ ТипПолученногоЗначения = Тип("Дата") Тогда
		рез = Значение;
	ИначеЕсли Справочники.ТипВсеСсылки().СодержитТип(ТипПолученногоЗначения)
				 ИЛИ Документы.ТипВсеСсылки().СодержитТип(ТипПолученногоЗначения)
				 ИЛИ ПланыВидовХарактеристик.ТипВсеСсылки().СодержитТип(ТипПолученногоЗначения)
				 ИЛИ ПланыОбмена.ТипВсеСсылки().СодержитТип(ТипПолученногоЗначения) Тогда
		рез = xmlСтрока(Значение);
	ИначеЕсли Перечисления.ТипВсеСсылки().СодержитТип(ТипПолученногоЗначения) Тогда
		// Получим строку вида Перечисление.ФизЮрЛицо.ФизЛицо
		рез = Значение.Метаданные().ПолноеИмя() + "." + xmlСтрока(Значение);
	Иначе
		рез = Строка(Значение);
	КонецЕсли;
...
Функция ПолучитьСсылкуПоИД(ИД, Тип = Неопределено) Экспорт
	Если ИД = "" и Тип <> Неопределено Тогда
		...
		переч = Метаданные.Перечисления.Найти(Тип);
		Если переч <> Неопределено Тогда
			Возврат переч[Тип].ПустаяСсылка();
		КонецЕсли;

		ПВХ = Метаданные.ПланыВидовХарактеристик.Найти(Тип);
		Если ПВХ <> Неопределено Тогда
			Возврат ПВХ[Тип].ПустаяСсылка();
		КонецЕсли;
	...
	Если стрНайти(ИД, ".") > 0 Тогда
		// это перечисление. Ждём что в поле ИД будет нечто вида Перечисление.ФизЮрЛицо.ФизЛицо
		// Из-за двуязывчного представления молжет быть и так Enum.ФизЮрЛицо.ФизЛицо
		// поэтому мы исходим из того, что в перечислении две точки
		
		// может быть ещё вариант когда в параметр Тип нам передадут имя перечисления,
		// а в параметр ИД строку со значением перечисления, но мы пока это не обрабатываем
		// Когда-нибудь, наверное, стоит сделать 
		
		мас = стрРазделить(ИД, ".");
		// мас[0] будет Перечисление
		// мас[1] будет ФизЮрЛицо
		// мас[2] будет ФизЛицо
		Если мас.Количество() = 3 Тогда
			переч = Метаданные.Перечисления.Найти(мас[1]);
			Если переч = Неопределено Тогда
				Возврат Неопределено;
			КонецЕсли;
			Если переч.ЗначенияПеречисления.Найти(мас[2]) = Неопределено Тогда
				Возврат Перечисления[мас[1]].ПустаяСсылка(); // не нашли значения
			Иначе
				Возврат Перечисления[мас[1]][мас[2]]; // вернём найденное значение перечисления
			КонецЕсли;
		КонецЕсли;		
	КонецЕсли;
	...
	Если Тип <> Неопределено Тогда
		...
		мд = Метаданные.ПланыВидовХарактеристик.Найти(Тип);
		Если мд <> Неопределено Тогда
			Ссылка = ПланыВидовХарактеристик[Тип].ПолучитьСсылку(УИД);
			Если Не Ссылка.Пустая() и НЕ ПустаяСтрока(Ссылка.ВерсияДанных) Тогда
				Возврат Ссылка
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого мд из Метаданные.ПланыВидовХарактеристик Цикл
		Ссылка = ПланыВидовХарактеристик[мд.Имя].ПолучитьСсылку(УИД);
		Если НЕ ПустаяСтрока(Ссылка.ВерсияДанных) Тогда
			Возврат Ссылка
		КонецЕсли;
	КонецЦикла;
...
Функция ПровестиЗначениеКТипу(выбЗначение, ВыбТип) Экспорт
	...
	ИначеЕсли ВыбТип = Перечисления.AINSYS_ТипыДанных.Идентификатор Тогда
		резЗначение = AINSYS_сервер.ПолучитьСсылкуПоИД(выбЗначение);
	КонецЕсли;
